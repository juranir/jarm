...
class Jarm:
    ...
    def send_packet(self, packet):
        try:
            ...
            try:
                if (type(ipaddress.ip_address(self.host)) == ipaddress.IPv4Address) or (type(ipaddress.ip_address(self.host)) == ipaddress.IPv6Address):
                    raw_ip = True
            ...
            if ":" in self.host:
                ...
                #Timeout of 20 seconds by default
            ...
            sock.close()
            return bytearray(data), self.ip[0]
        #Timeout errors result in an empty hash
        except socket.timeout as e:
            sock.close()
            return "TIMEOUT", self.ip[0]
        except Exception as e:
            sock.close()
            return None, self.ip[0]
    ...
    def main(self):
        ...
        #Array format = [self.host,self.port,version,cipher_list,cipher_order,GREASE,RARE_APLN,1.3_SUPPORT,extension_orders]
        tls1_2_forward = [self.host, self.port, "TLS_1.2", "ALL", "FORWARD", "NO_GREASE", "APLN", "1.2_SUPPORT", "REVERSE"]
        tls1_2_reverse = [self.host, self.port, "TLS_1.2", "ALL", "REVERSE", "NO_GREASE", "APLN", "1.2_SUPPORT", "FORWARD"]
        tls1_2_top_half = [self.host, self.port, "TLS_1.2", "ALL", "TOP_HALF", "NO_GREASE", "APLN", "NO_SUPPORT", "FORWARD"]
        tls1_2_bottom_half = [self.host, self.port, "TLS_1.2", "ALL", "BOTTOM_HALF", "NO_GREASE", "RARE_APLN", "NO_SUPPORT", "FORWARD"]
        tls1_2_middle_out = [self.host, self.port, "TLS_1.2", "ALL", "MIDDLE_OUT", "GREASE", "RARE_APLN", "NO_SUPPORT", "REVERSE"]
        tls1_1_middle_out = [self.host, self.port, "TLS_1.1", "ALL", "FORWARD", "NO_GREASE", "APLN", "NO_SUPPORT", "FORWARD"]
        tls1_3_forward = [self.host, self.port, "TLS_1.3", "ALL", "FORWARD", "NO_GREASE", "APLN", "1.3_SUPPORT", "REVERSE"]
        tls1_3_reverse = [self.host, self.port, "TLS_1.3", "ALL", "REVERSE", "NO_GREASE", "APLN", "1.3_SUPPORT", "FORWARD"]
        tls1_3_invalid = [self.host, self.port, "TLS_1.3", "NO1.3", "FORWARD", "NO_GREASE", "APLN", "1.3_SUPPORT", "FORWARD"]
        tls1_3_middle_out = [self.host, self.port, "TLS_1.3", "ALL", "MIDDLE_OUT", "GREASE", "APLN", "1.3_SUPPORT", "REVERSE"]
        ...
        while iterate < len(queue):
            ...
            server_hello, self.ip = self.send_packet(payload)
